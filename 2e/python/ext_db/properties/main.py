...
class MainPage(webapp2.RequestHandler):
    def get(self):
        ...
        try:
            ...
            self.response.write('''
<p>I cannot create a MyModel without initializing p2, which is
...
''')
        ...
        try:
            ...
            self.response.write('''
<p>I cannot create a MyModel with p4='Fall', since that is not
...
''')
        ...
        try:
            ...
            self.response.write('''
<p>I cannot create a MyModel with p6=1909, since that is not a
valid value according to the custom validator function.</p>
''')
        
        obj = MyModel(p2='value', p4='Autumn', p6=1978)
        self.response.write('''
<p>I can create a MyModel with p2='value', p4='Autumn' and p6=1978,
...
''')
        ...
        self.response.write('<p>Property values of this object after '
                            'initialization:</p><ul>')
        prop_names = MyModel.properties().keys()
        prop_names.sort()
        for p in prop_names:
            self.response.write('<li>%s = %s</li>' % (p, repr(getattr(obj, p))))
        self.response.write('</ul>')
        
        self.response.write('<p>Assigning a non-empty list to '
                            'integer_list_prop...</p>')
        obj.integer_list_prop = [ 2, 4, 6, 8 ]
        
        self.response.write('<p>Overriding automatic values for '
                            'auto_now_add_prop and auto_now_prop...</p>')
        obj.auto_now_add_prop = datetime.datetime(1999, 12, 31, 23, 59, 59, 0)
        obj.auto_now_prop = datetime.datetime(1999, 12, 31, 23, 59, 59, 0)
        
        self.response.write('<p>Saving...</p>')
        obj.put()
